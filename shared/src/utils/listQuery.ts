import type { EntityListQuery, ProcessStage, Role, StockItemKind } from "../types/index.js";
...
export function buildListQueryString(query: EntityListQuery = {}): string {
  ...
  if (n.itemKind && n.itemKind !== "all") params.set("itemKind", n.itemKind);
  ...
  if (n.target) params.set("target", n.target);
...
export function parseListQueryFromRequest(
  ...
  return {
    ...
    itemKind: q.itemKind as StockItemKind | "all" | undefined,
    status: q.status,
    orderId: q.orderId,
    bomId: q.bomId,
